import getPatternsForRow from "./getRegexForRow.mjs"
// import commonWords from "../../common/wordLists/compiled/commonWords.json"; todo
...
function generateGrid(gridSize, minLetters, minWordLength) {
  let grid =Array.from({ length: gridSize }, () => (Array.from({ length: gridSize }, () => (""))))

  
const startingWord = "FLOWER" // todo pick randomly
let letterCount = startingWord.length

// Pick a random starting position
const startingRowIndex = Math.floor(Math.random() * gridSize)
const startingColIndex = Math.floor(Math.random() * gridSize)

// inject the starting word into the starting array
for (let index = startingColIndex; index < startingColIndex + startingWord.length; index++) {
...
while (letterCount < minLetters && count < 30) {
  ...
  for (let metaIndex = 0; metaIndex < indexesByCounts.length; metaIndex++) {
    let matchingWord
    let startPosition
    const patterns = getPatternsForRow(grid, indexesByCounts[metaIndex], minWordLength)
    
    for (let patternIndex = 0; patternIndex < patterns.length; patternIndex++) {
      matchingWord = commonWords.find((word) => word.match(patterns[patternIndex][0]))
      if (matchingWord) {
        startPosition = patterns[patternIndex][1]

        // inject the word
      for (let index = startPosition; index < startPosition + matchingWord.length; index++) {
        grid[indexesByCounts[metaIndex]][index] = matchingWord[index - startPosition]
      }
