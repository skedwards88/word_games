...
test("word matches pattern and is known word", () => {
  ...
  expect(isValidGuess({word: "CAMPERS", pattern: "[A-Z]+M[A-Z]+RS$"})).toEqual(
    true,
  );
  expect(isValidGuess({word: "CAMPERS", pattern: "[A-Z]+MP[A-Z]+S$"})).toEqual(
    true,
  );
  expect(isValidGuess({word: "CAMPERS", pattern: "[A-Z]+MPE[A-Z]+"})).toEqual(
    true,
  );
  expect(isValidGuess({word: "CAMPERS", pattern: "^C[A-Z]+P[A-Z]+S$"})).toEqual(
    true,
  );
  expect(isValidGuess({word: "CAMPERS", pattern: "^C[A-Z]+PE[A-Z]+"})).toEqual(
    true,
  );
  expect(isValidGuess({word: "CAMPERS", pattern: "^CA[A-Z]+E[A-Z]+"})).toEqual(
    true,
  );
  expect(
...
test("word does not match pattern and is known word", () => {
  ...
  expect(isValidGuess({word: "HAPPINESS", pattern: "[A-Z]+MPE[A-Z]+"})).toEqual(
    false,
  );
  expect(isValidGuess({word: "CAMPER", pattern: "^C[A-Z]+P[A-Z]+S$"})).toEqual(
    false,
  );
  expect(isValidGuess({word: "SCAMPERS", pattern: "^C[A-Z]+PE[A-Z]+"})).toEqual(
    false,
  );
  expect(isValidGuess({word: "SCAMPERS", pattern: "^CA[A-Z]+E[A-Z]+"})).toEqual(
    false,
  );
  expect(
